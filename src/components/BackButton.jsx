...
const BackButton = () => {
    ...
    const back = () => {
        ...
        let controlsTargetOffset = [-.01, 0, 0], cameraPosition = [0,1.5,0]

        if ([FOCUS_FACILITIES, FOCUS_SUBJECT, FOCUS_RESEARCH].includes(focusTarget)) {
            controlsTargetOffset = [0, 0, -.01]
        }

        if ([FOCUS_LECTURER, FOCUS_COMMUNITY_SERVICE, FOCUS_BOOKS].includes(focusTarget)) {
            controlsTargetOffset = [0, 0, .01]
        }
